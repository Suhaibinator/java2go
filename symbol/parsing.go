...
func parseClassScope(root *sitter.Node, source []byte) *ClassScope {
	...
	for _, node := range nodeutil.NamedChildrenOf(root.ChildByFieldName("body")) {
		parseClassBodyNode(node, source, scope)
	}

	return scope
}

func parseClassBodyNode(node *sitter.Node, source []byte, scope *ClassScope) {
	switch node.Type() {
	case "enum_constant":
		// Handle enum constant
		// Treat as public static field of type EnumClass
		name := node.ChildByFieldName("name").Content(source)
		scope.Fields = append(scope.Fields, &Definition{
			Name:         HandleExportStatus(true, name), // Always public
			OriginalName: name,
			Type:         scope.Class.Name,         // Type is the enum itself
			OriginalType: scope.Class.OriginalName, // Type is the enum itself
		})
	case "enum_body_declarations":
		for _, child := range nodeutil.NamedChildrenOf(node) {
			parseClassBodyNode(child, source, scope)
		}
